...
const IconSearchPopover = withState( {
	...
	return (
		<Popover
			className="ahg-icon-popover"
			onClose={ props.onClose }
			onClickOutside={ props.onClickOutside }
		>
			<PanelBody>
				<div className="ahg-icon-popover__label-container">
					<TextControl
						className="ahg-icon-popover__input"
						value={ value }
						onChange={ value => {
							setState( { value } );
						} }
						placeholder={ __( 'Type to search icon' ) }
					/>
					{ props.allowReset &&
						<Button
							onClick={ () => {
				...
				<div className="ahg-icon-popover__iconlist">
					...
					{ ! results.length &&
						<p className="components-base-control__help">{ __( 'No matches found' ) }</p>
					}
				</div>
			</PanelBody>
		</Popover>
